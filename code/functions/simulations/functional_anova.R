functional_anova <- function(
  ...
  if (bootstrap == FALSE) {
    ...
    bootstrap_test_stats <- map(
      ...
      ~ replicate(n_bootstrap, matrix(NA, nrow = nrow(.x), ncol = ncol(.x)))
    )
    ...
    for (partition_idx in seq_along(critical_values)) {
      nrows <- dim(bootstrap_test_stats[[partition_idx]])[1]
      ncols <- dim(bootstrap_test_stats[[partition_idx]])[2]

      for (row_idx in seq_len(nrows)) {
        for (col_idx in seq_len(ncols)) {
          critical_values[[partition_idx]][row_idx, col_idx] <- quantile(
            bootstrap_test_stats[[partition_idx]][row_idx, col_idx, ],
            probs = 1 - alpha,
            na.rm = TRUE
          )
        }
