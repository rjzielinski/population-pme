functional_permutation_test <- function(
  ...
  # if any_reject = TRUE, reject the null hypothesis if
  # rejected along any dimension. Use the maximum test statistic across all
  # dimensions in permutations to protect family-wise error rate
  ...
  f_permute_max_rejected <- list()
  l2_norm_permute_rejected <- list()
  l2_norm_permute_max_rejected <- list()

  f_permute_p_values <- list()
  f_permute_max_p_values <- list()
  l2_norm_permute_p_values <- list()
  l2_norm_permute_max_p_values <- list()
  ...
  f_permute_test_stat_max <- map(
    f_permute_test_stats,
    ~ apply(.x, c(1, 3), max)
  )

  l2_norm_permute_test_stat_max <- map(
    l2_norm_permute_test_stats,
    ~ rowMaxs(.x, value = TRUE)
  )
  ...
  f_permute_max_critical_values <- map(
    f_permute_test_stat_max,
    ~ vector(mode = "numeric", length = nrow(.x))
  )
  ...
  l2_norm_permute_max_critical_values <- map(
    l2_norm_permute_test_stat_max,
  ...
  l2_norm_permute_max_p_vals <- map(
    l2_norm_permute_test_stats,
    ~ vector(mode = "numeric", length = ncol(.x))
  )
  ...
  for (partition_idx in seq_len(n_partitions)) {
    ...
    for (row_idx in seq_len(nrows)) {
      f_permute_max_critical_values[[partition_idx]][row_idx] <- quantile(
        f_permute_test_stat_max[[partition_idx]][row_idx, ],
        probs = 1 - alpha,
        na.rm = TRUE
      )
      ...
      for (col_idx in seq_len(ncols)) {
        ...
        f_permute_max_p_vals[[partition_idx]][row_idx, col_idx] <- 1 -
          mean(
            f_test_stat[[partition_idx]][row_idx, col_idx] >
              f_permute_test_stat_max[[partition_idx]][row_idx, ],
            na.rm = TRUE
    ...
    for (col_idx in seq_len(ncols)) {
      ...
      l2_norm_permute_max_p_vals[[partition_idx]][col_idx] <- 1 -
        mean(
          l2_norm_test_stat[[partition_idx]][col_idx] >
            l2_norm_permute_test_stat_max[[partition_idx]],
          na.rm = TRUE
        )
    }
  }
  ...
  for (partition_idx in seq_len(n_partitions)) {
    ...
    f_permute_max_rejected[[partition_idx]] <- map(
      seq_len(ncol(f_test_stat[[partition_idx]])),
      ~ f_test_stat[[partition_idx]][, .x] >
        f_permute_max_critical_values[[partition_idx]]
    )
    f_permute_max_p_values[[partition_idx]] <- f_permute_max_p_vals[[
      partition_idx
    ]]
    ...
    l2_norm_permute_max_rejected[[partition_idx]] <- l2_norm_test_stat[[
      partition_idx
    ]] >
      l2_norm_permute_max_critical_values[[partition_idx]]
    l2_norm_permute_max_p_values[[
      partition_idx
    ]] <- l2_norm_permute_max_p_vals[[partition_idx]]
  }
  ...
  f_test <- list(
    ...
    permute_max_critical_values = f_permute_max_critical_values,
    permute_max_rejected = f_permute_max_rejected,
    permute_max_p_values = f_permute_max_p_values,
  ...
  l2_norm_test <- list(
    ...
    permute_max_critical_values = l2_norm_permute_max_critical_values,
    permute_max_rejected = l2_norm_permute_max_rejected,
    permute_max_p_values = l2_norm_permute_max_p_values,
...
calc_embeddings <- function(
  ...
  for (partition_idx in seq_along(additive_model)) {
    ...
    group_means[[partition_idx]] <- foreach(
      ...
      {
        full_embeddings[id_groups == group_vals[group_idx]] |>
          reduce(`+`) /
          sum(id_groups == group_vals[group_idx])
      }
